...
function GameTuto3() {
    ...
    var canvTimeline = document.getElementById("timelineCanvasTuto3");
    ...
    //set up game canvas 
    var canv = document.getElementById("formsBoardCanvasTuto3");
    canv.height = HEIGHT;
    canv.width = WIDTH;
            ...
            for (let form of row) {
                if (form.contains(x, y) && !form.selected) {
                    if (!form.selectable) {
                        ...
                        if (learningState[form.constructor.name] == NB_LOCKS3) {
                            selectAllUnlockForm = form.constructor.name;
                            document.getElementById("tuto3step5").style.color = "#4CAF50";
                        } else if (nbCurrentFormSelected >= nbFormToSelect) {
                            document.getElementById("nextButtonTuto3").disabled = false;
                            document.getElementById("tuto3step1").style.color = "#4CAF50";
                        }
                        break OUTER; //if one form is to highlight no need to look further
                    }
                }
    ...
    function gameUpdate() {
        ...
        if (currentStep >= STEP) {
            ...
            document.getElementById("boardTuto3").style.display = 'none';
            const conditions = document.getElementsByClassName("condition");
            var conditionFullfiled = true;
            Array.prototype.forEach.call(conditions, condition => {
                if (condition.style.color != "rgb(76, 175, 80)") {
                    document.getElementById("buttonRedoTuto3").style.display = '';
                    document.getElementById("boardTuto3").style.display = 'none';
            ...
            if (conditionFullfiled) {
                document.getElementById("buttonRedoTuto3").style.display = '';
                document.getElementById("endTuto").style.display = '';
            }
            clearInterval(gameRunning);
            learningState['Square'] = 0;
            return;
        }
        ...
        boardInfos = newGame(nameCurrentForm); //learningState[nameCurrentForm] == NB_LOCKS3 ? 'nothing to select' : nameCurrentForm);
    ...
    */
    ...
    var targetCanvas = document.getElementById("targetCanvasTuto3");
    ...
    function unlocker( /* type MouseEvent*/ event) {
        ...
        let x = event.offsetX;
        let y = event.offsetY;
        if (unlockButton.contains(x, y) && nbCurrentFormSelected >= nbFormToSelect && learningState[nameCurrentForm] != NB_LOCKS3) {
            unlockButton = null;
            sliderDisplay();
            document.getElementById("tuto3step2").style.color = "#4CAF50";
        }
    }
    ...
    function sliderDisplay() {
        ...
        document.getElementById("boardTuto3").appendChild(slider);
    }
    ...
    function unlock() {
        ...
        if (learningState[nameCurrentForm] == NB_LOCKS3) return;
        //next if is to test if learning state can improve
        if (learningState[nameCurrentForm] < NB_LOCKS3) {
            learningState[nameCurrentForm] += 1;
            unlockable = false;
            unlockDone = 1; //for listTryUnlock
            var timestamp = Date.now();
            currentUnlockTime = (timestamp - start);
            document.getElementById("tuto3step3").style.color = "#4CAF50";
            if (learningState[nameCurrentForm] == 2) {
                document.getElementById("tuto3step4").style.color = "#4CAF50";
            }
            //nbLockOpened++;
        }
    ...
    function drawTarget() {
        ...
        ctxTarget.textAlign = "center";
        var text = "UNLOCK";
        if (learningState[nameCurrentForm] == NB_LOCKS3) text = "UNLOCKED";
    ...
    const LC_HEIGHT = LC_CELL * (3 / 2) + LC_MARGIN / 2; //LC_CELL * (formsList.length + 1) + LC_MARGIN / 2; //LearningCanvas Height
    const LC_WIDTH = 3 / 2 * LC_CELL + parseInt(NB_LOCKS3) * (IMG_WIDTH + IMG_MARGIN / 3); //LearningCanvas Width
    ...
    var learningCanvas = document.getElementById("learningCanvasTuto3");
    ...
    function drawLocks() {
        for (let i = 0; i < 1; i++) { //formsList.length; i++) {
            for (let j = 0; j < NB_LOCKS3; j++) {
                if (learningState[formsList[i]] > j || learningState[formsList[i]] == NB_LOCKS3) {
    ...
    var nextButton = document.getElementById("nextButtonTuto3");
    ...
    nextButton.style.marginRight = String(WIDTH / 2 + STROKE + 50) + "px";;
    nextButton.disabled = true;
    ...
    const gameRunning = setInterval(loop, 1000 / FPS);
